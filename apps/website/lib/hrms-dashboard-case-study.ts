export const dashboardCaseStudy = {
  metadataTitle: "HR Analytics Dashboard — Timelabs | Rishabh Choudhary",
  metadataDescription:
    "How I redesigned the Timelabs HRMS admin dashboard into an analytics platform that surfaces attendance patterns and workforce insights for enterprise leaders.",
  title: "An HR analytics dashboard for an HRMS industry leader",
  role: "UX Designer",
  company: "Timelabs",
  year: "2023–2024",
  timeline: "~2–3 weeks design · ~1 month build, QA, and launch",
  team: "Rishabh (design), a data engineer, a frontend developer, stakeholders (PMs, CTO, CEO)",
  tools: "Figma",
  toc: [
    { id: "snapshot", label: "Snapshot" },
    { id: "problem", label: "The problem" },
    { id: "stakeholders", label: "Working with stakeholders" },
    { id: "kpis", label: "Picking what to measure" },
    { id: "charts", label: "Choosing the right charts" },
    { id: "engineering", label: "Making it real" },
    { id: "qa-bug", label: "The bug I caught" },
    { id: "outcome", label: "Where it landed" },
    { id: "reflection", label: "Reflection" },
  ],
};
